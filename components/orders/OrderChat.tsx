...
export default function OrderChat({ orderId, currentUserId, isActive }: OrderChatProps) {
  ...
  const unread = Math.max(0, messages.length - (seenCountByOrder[orderId] ?? 0));
  ...
  useEffect(() => {
    if (open) {
      markSeen(orderId, messages.length);
    }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-6 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {unread > 0 && (
              <span className="text-xs bg-[#FF3008] text-white px-2 py-0.5 rounded-full font-medium">{unread}</span>
            )}
